...
def bort(vertex, left, right):
    ...
    if len(leftSuccessors) == 1:
        ...
        raise BrokenPrecondition(f"More than one left successor found for vertex {vertex} in edge list {left}")
    elif len(leftSuccessors) == 0:
        raise BrokenPrecondition(f"No left successor found for vertex {vertex}")

    if len(rightSuccessors) == 1:
        rightChild = leftSuccessors.pop()
    elif len(rightSuccessors) > 1:
        raise BrokenPrecondition(f"More than one right successor found for vertex {vertex} in edge list {right}")
    elif len(rightSuccessors) == 0:
        raise BrokenPrecondition(f"No right successor found for vertex {vertex}")
